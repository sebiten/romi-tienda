...
function getStatusBadge(status: string) {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-1 text-xs font-semibold rounded-full border ${config.class}`}>
      {config.label}
    </span>
  )
}
    ...
    <main className="bg-beige-50 min-h-screen py-8 px-4">
      <div className="container mx-auto max-w-7xl">
        ...
        <header className="mb-6 flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="font-serif text-3xl md:text-4xl text-beige-800 mb-2 leading-tight">Gestión de Pedidos</h1>
            <p className="text-beige-600 text-sm md:text-base">Administra y visualiza todos los pedidos</p>
          </div>
          <Link href="/admin">
            <Button variant="outline" className="border-beige-300 text-beige-700 hover:bg-beige-100 bg-transparent">
              Volver al Dashboard
            </Button>
          </Link>
        </header>
            ...
            {orders?.length ? (
              <div className="space-y-4">
                {orders.map((order: Order) => (
                  <Card key={order.id} className="border border-beige-200 shadow-sm hover:shadow-md transition-shadow">
                    <CardContent className="p-4 md:p-5">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
                        ...
                        <div className="text-right">
                          <p className="text-xs font-medium text-beige-600 mb-1">Total</p>
                                  ...
                                  >
                                    <div className="flex-1">
                                      ...
                                      <div className="flex flex-wrap gap-2 text-xs text-beige-600">
                                        <span className="bg-beige-100 px-2 py-1 rounded">Cant: {item.quantity}</span>
                                        {item.size && (
                                          <span className="bg-beige-100 px-2 py-1 rounded">Talle: {item.size}</span>
                                        )}
                                        {item.color && (
                                          <span className="bg-beige-100 px-2 py-1 rounded">Color: {item.color}</span>
                                        )}
                                      </div>
                                    </div>
                                    ...
                                    <div className="text-right">
                                      ...
                                      <p className="font-semibold text-beige-900">
                                        $
                                        {(item.quantity * (item.unit_price ?? item.product?.price ?? 0)).toLocaleString(
                                          "es-AR",
                                        )}
                                      </p>
                                    </div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-2">
                        <Button asChild className="flex-1 bg-beige-700 hover:bg-beige-800 text-white">
              ...
              <div className="text-center py-12">
                ...
                <p className="text-beige-600 font-medium">No hay pedidos registrados</p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {currentPage <= 1 ? (
                      ...
                      <Link href={`/admin/pedidos?page=${currentPage - 1}`} className="flex items-center gap-1">
                        <ChevronLeft className="h-4 w-4" />
                        Anterior
                      </Link>
                    )}
