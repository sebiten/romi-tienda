...
import { User, Package, Heart, Settings, LogOut, Edit, ShoppingBag, MapPin, Calendar } from "lucide-react"
...
export default async function Perfil() {
  ...
  const avatar = user.user_metadata?.avatar_url || "/placeholder.svg?height=150&width=150"
  ...
  const mockOrders = [
    { id: "ORD-1234", date: "15 Mar 2023", status: "Entregado", total: "$1,299" },
        ...
        <div className="absolute inset-0 opacity-10">
          <svg className="h-full w-full" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg">
            <pattern id="grid" width="80" height="80" patternUnits="userSpaceOnUse">
              <path d="M 80 0 L 0 0 0 80" fill="none" stroke="#9C8772" strokeWidth="0.5" />
            </pattern>
              ...
              <div className="flex flex-col items-center">
                <div className="relative group">
                  ...
                  <div className="relative w-24 h-24 rounded-full overflow-hidden border-4 border-white">
                    <Image src={avatar || "/placeholder.svg"} alt={fullName} fill className="object-cover" />
                ...
                <h1 className="mt-4 text-2xl font-serif text-beige-800">{fullName}</h1>
          ...
          <div className="lg:col-span-2">
            <Card className="bg-white border-beige-200 shadow-sm mb-8">
              <CardHeader className="pb-2">
                <h2 className="text-xl font-serif text-beige-800">Información Personal</h2>
                <p className="text-sm text-beige-600">Gestiona tu información personal y cuenta</p>
              </CardHeader>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-beige-700 mb-1">Correo Electrónico</label>
                      <div className="p-2 bg-beige-50 rounded border border-beige-200 text-beige-800">{email}</div>
                    </div>
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-beige-700 mb-1">ID de Usuario</label>
                    ...
                    {mockOrders.length > 0 ? (
                      <div className="divide-y divide-beige-100">
                        {mockOrders.map((order) => (
                          <div key={order.id} className="p-4 hover:bg-beige-50 transition-colors">
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center">
                                <ShoppingBag className="w-4 h-4 text-beige-500 mr-2" />
                                <span className="font-medium text-beige-800">{order.id}</span>
                              </div>
                            ...
                            <div className="flex justify-between text-sm">
                              ...
                              <span className="font-medium text-beige-700">{order.total}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="p-8 text-center">
                        ...
                        <h3 className="text-lg font-medium text-beige-800 mb-1">No hay pedidos</h3>
                        <p className="text-beige-600 mb-4">Aún no has realizado ningún pedido.</p>
                    ...
                    {mockAddresses.length > 0 ? (
                      <div className="space-y-4">
                        {mockAddresses.map((address) => (
                          <div key={address.id} className="p-4 border border-beige-200 rounded-md relative">
                            {address.isDefault && (
                              <span className="absolute top-2 right-2 text-xs bg-beige-100 text-beige-700 px-2 py-0.5 rounded-full">
                                Predeterminada
                              </span>
                            )}
                            <h3 className="font-medium text-beige-800">{address.name}</h3>
                            <p className="text-beige-600 mt-1">{address.address}</p>
                      ...
                      <div className="p-8 text-center">
                        ...
                        <h3 className="text-lg font-medium text-beige-800 mb-1">No hay direcciones</h3>
                        <p className="text-beige-600 mb-4">Aún no has añadido ninguna dirección de envío.</p>
                        <Button className="bg-beige-700 hover:bg-beige-800 text-beige-50">Añadir dirección</Button>
                      </div>
                    )}
                  </CardContent>
                </Card>
...
function Badge({ status }: { status: string }) {
  ...
  return <span className={`text-xs px-2 py-1 rounded-full ${bgColor} ${textColor}`}>{status}</span>
}
